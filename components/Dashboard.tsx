...
const CopingBar = ({ onStart }: { onStart: (id: string) => void }) => {
  ...
  return (
    <div className="grid grid-cols-4 gap-3 px-2 pb-8">
      {tools.map(tool => (
        <button 
          key={tool.id}
          onClick={() => onStart(tool.id)}
          className="aspect-square bg-slate-900/40 dark:bg-slate-900/80 backdrop-blur-xl border-2 border-white/5 dark:border-slate-800 rounded-[32px] md:rounded-[44px] flex flex-col items-center justify-center gap-2 md:gap-3 active:scale-95 transition-all shadow-2xl group/tool"
        >
          <span className="text-xl md:text-3xl group-hover/tool:scale-110 transition-transform duration-500">{tool.icon}</span>
          <span className="text-[9px] md:text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] group-hover/tool:text-white transition-colors">{tool.label}</span>
        </button>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-1000 pb-32">
      ...
      <section className="bg-slate-950 rounded-[64px] border-4 border-white/5 shadow-2xl overflow-hidden relative min-h-[520px] flex flex-col items-center justify-center text-center p-8 md:p-12" aria-labelledby="welcome-heading">
        ...
        <div className="relative z-10 space-y-10 w-full max-w-4xl">
          <div className="space-y-8">
             <div className="flex flex-col items-center gap-4">
               ...
               <h1 id="welcome-heading" className="text-4xl md:text-7xl font-black text-white tracking-tighter leading-none">
                 This is your Footpath to Freedom, <span className="bg-gradient-to-r from-teal-400 to-indigo-400 bg-clip-text text-transparent">{user?.name.split(' ')[0] || 'Traveller'}</span>
               </h1>
             </div>
             ...
             <div className="space-y-3">
                ...
                <p className="text-slate-300 text-lg md:text-xl font-medium italic opacity-90 leading-relaxed max-w-xl mx-auto font-serif">
                  "Every breath is a fresh start, every choice a new horizon. The old paths are fading; your True-Self is rising."
                </p>
             </div>
          ...
          <div className="flex flex-wrap justify-center gap-6 md:gap-16 border-t border-white/5 pt-10 px-4">
             ...
             <div className="text-center group">
                <p className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em] mb-2 group-hover:text-teal-400 transition-colors">Path Discovered</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 px-2">
        ...
        <section className="md:col-span-8 h-[360px]">
          <EngagementProgress 
            moods={moods} 
            goals={goals} 
        ...
        {/* Strategic Marker Widget (Consistent with Bio Pulse) */}
        <section 
          className="md:col-span-4 bg-indigo-950/40 backdrop-blur-3xl p-10 rounded-[56px] border border-white/10 shadow-2xl flex flex-col justify-between group transition-all relative overflow-hidden h-[340px]"
          aria-labelledby="goal-title"
        >
           <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/5 rounded-full -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-[2000ms]" />
           <div className="space-y-6 relative z-10 flex-grow">
              <div className="flex justify-between items-start">
                 <div className="w-14 h-14 bg-white/5 text-indigo-300 rounded-2xl border border-white/10 flex items-center justify-center text-2xl shadow-inner group-hover:rotate-6 transition-transform" aria-hidden="true">🧭</div>
                 <div className="text-right">
                    <h3 id="goal-title" className="text-xl font-black text-white tracking-tight">Active Marker</h3>
                    <span className="text-[10px] font-black text-white/30 uppercase tracking-widest" aria-hidden="true">STRATEGIC GOAL</span>
                 </div>
              </div>
              ...
              <div className="space-y-4">
                 <div className="p-6 bg-white/5 rounded-3xl border border-white/5 shadow-inner">
                    ...
                    {activeGoal && (
                       <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mt-2">Target: {activeGoal.timeBound}</p>
                    )}
                 </div>
                 
                 <button 
                   onClick={() => onSetRoute(AppRoute.SMART_GOALS)}
                   className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-2xl shadow-lg transition-all uppercase tracking-widest text-[11px] active:scale-95 border border-white/5"
                 >
                   {activeGoal ? "Update Progress" : "Set First Marker"}
                 </button>
              </div>
           </div>
        </section>
        ...
        {/* Freedom Wealth (Consistent Row Height) */}
        <section 
          className="md:col-span-4 bg-emerald-950/40 backdrop-blur-3xl p-10 rounded-[56px] border border-white/10 shadow-2xl text-white flex flex-col justify-between group active:scale-[0.98] transition-all relative overflow-hidden h-[340px]" 
