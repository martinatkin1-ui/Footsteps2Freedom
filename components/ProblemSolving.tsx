...
const STEPS = [
  { id: 'S', title: 'Specify the Problem', prompt: 'Be specific. Ask yourself: "Why is this a problem for me?"', icon: '🎯' },
  { id: 'O', title: 'Options', prompt: 'Add any ideas you can think of. Do NOT evaluate them at this stage.', icon: '💡' },
  { id: 'L', title: 'List Pros & Cons', prompt: 'What are the likely benefits and disadvantages of each option?', icon: '⚖️' },
  { id: 'V', title: 'Verify the Solution', prompt: 'Choose the best solution and write out your action plan.', icon: '✅' },
  { id: 'E', title: 'Evaluate', prompt: 'Imagine the result. What went well? What might not go well?', icon: '🔍' }
];
...
const ProblemSolving: React.FC<ProblemSolvingProps> = ({ onComplete, onAskGuide }) => {
  ...
  const handleNext = async () => {
    if (step < STEPS.length - 1) {
      ...
      const res = await getModuleReflection("SOLVE Model", `User solved a problem: ${data[0]}. Solution picked: ${data[3]}`);
      setReflection(res);
      setShowReflection(true);
    }
  };

  if (showReflection) return <ModuleReflection moduleName="SOLVE Model" context="Problem decoding complete." reflection={reflection} onClose={onComplete} title="Problem Decoded" />;
    ...
    <div className="max-w-2xl mx-auto space-y-10 animate-in fade-in duration-500 pb-32">
      <div className="flex justify-between items-center px-4">
        <div className="flex gap-2 flex-grow max-w-xs">
          {STEPS.map((_, i) => (
            <div key={i} className={`h-2 flex-grow rounded-full transition-all duration-700 ${i <= step ? 'bg-indigo-600 shadow-sm' : 'bg-slate-200 dark:bg-slate-800'}`} />
          ))}
        </div>
        {onAskGuide && (
          <button 
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[40px] p-8 md:p-16 border-2 border-slate-100 dark:border-slate-800 shadow-xl relative overflow-hidden group">
        <div className="absolute top-0 right-0 w-80 h-80 bg-indigo-500/5 rounded-full -mr-40 -mt-40 blur-3xl opacity-60" />
        
        <div className="relative z-10 space-y-10">
          <div className="flex items-center gap-8">
             <div className="w-20 h-20 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-[28px] flex items-center justify-center text-4xl shadow-inner border-2 border-indigo-100 dark:border-indigo-800 font-black">{STEPS[step].id}</div>
             <div className="space-y-1">
                <span className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-[0.4em]">Protocol Linked</span>
          ...
          <div className="relative group/input">
            <textarea 
              ...
              placeholder={isListening ? "Listening to your thoughts..." : "Record tactical observation..."}
          ...
          <div className="flex gap-4">
             ...
             <button 
              ...
              >
                {step === 4 ? "Complete Analysis" : "Next Protocol"}
              </button>
          </div>
        </div>
      </div>
